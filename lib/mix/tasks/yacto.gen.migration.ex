  ...
  def run(args) do
    ...
    case OptionParser.parse(args, switches: @switches) do
      {opts, [], _} ->
        ...
        ignore_schemas = Application.get_env(:yacto, :ignore_migration_schemas, []) |> MapSet.new()
        ...
        if length(dup_schemas) != 0 do
          for {base, schemas} <- dup_schemas do
            Logger.error("スキーマのベース名が重複しています。#{inspect(schemas)} が同じベース名 #{inspect(base)} を利用しています。")
          end

...
For example, when the module name is `MyApp.Foo.Models.Bar.Player`, `@auto_source` is `"my_app_foo_models_bar_player"` by default.
...
If this config is set, the `@auto_source` will be `"my_app_bar_player"`.
